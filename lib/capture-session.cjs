...
class CaptureSession {
  constructor({ assets, checkpoint, language, repoRoot, scenario }) {
    ...
    this.assets = assets.flatMap((asset) => {
  ...
  async navigationTitleAndFirstTable(page, checkpoint, options = {}) {
    await page.addStyleTag({ content: '#aside { visibility: hidden !important; }' });
    await this.shot(page, checkpoint, [
